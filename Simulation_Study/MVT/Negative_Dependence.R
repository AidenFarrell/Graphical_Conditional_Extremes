...
################################################################################
out <- readRDS("Data/MVT_Negative_Dependence_D5.RData")
...
dispersion_matrix <- Sigma_true*(df-2)/df
X <- replicate(n = n_sim,
               expr = mvtnorm::rmvt(n = n_data, sigma = dispersion_matrix, df = df, delta = mu_true),
               simplify = FALSE)
...
# Alpha
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Alpha.pdf", width = 15, height = 10)
boxplot_MLEs(
...
# Beta
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Beta.pdf", width = 15, height = 10)
boxplot_MLEs(
  data = lapply(1:d, function(i){do.call(cbind, lapply(1:p, function(j){
...
# Location
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Location.pdf", width = 15, height = 10)
boxplot_MLEs(
  data = lapply(1:d, function(i){do.call(cbind, lapply(1:p, function(j){
...
# Scale (Left)
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Scale_Left.pdf", width = 15, height = 10)
boxplot_MLEs(
  data = lapply(1:d, function(i){do.call(cbind, lapply(1:p, function(j){
...
# Scale (Right)
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Scale_Right.pdf", width = 15, height = 10)
boxplot_MLEs(
  data = lapply(1:d, function(i){do.call(cbind, lapply(1:p, function(j){
...
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Scale_Comp.pdf", width = 15, height = 15)
...
# Shape
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Shape.pdf", width = 15, height = 10)
boxplot_MLEs(
  data = lapply(1:d, function(i){do.call(cbind, lapply(1:p, function(j){
...
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Gamma_Bias.pdf", width = 15, height = 10)
...
pdf(file = "Images/Simulation_Study/MVT/Negative_Dependence/Gamma.pdf", width = 15, height = 10)
...
## true distribution
X_prob_calc <- mvtnorm::rmvt(n = 1e+7, sigma = dispersion_matrix, df = df, delta = mu_true)
...
################################################################################
## Get conditional cdf curves for all the univariate probabilities
cond_cdf_curve_data <- function(data, u_cond, u_dep, cond_var, dep_var){
  
  ## Subset the data
  data_cond_large <- data[which(data[,cond_var] > u_cond),]
  
  ## Number of excesses above the threshold
  n_data_cond_large <- nrow(data_cond_large)
  
  ## Conditional survivor curve
  cond_cdf_out <- sapply(u_dep, function(v){length(which(data_cond_large[,dep_var] < v))/n_data_cond_large})
  return(cond_cdf_out)
}

cond_cdf_curve_model <- function(data, u_cond, u_dep, cond_var, dep_var){
  ...
  n_data <- length(data)
  ## Subset the data
  data_cond_large <- lapply(data, function(x){x[which(x[,cond_var] > u_cond),]})
  
  ## Number of excesses above the threshold
  n_data_cond_large <- sapply(data_cond_large, nrow)
  
  ## Conditional survivor curve
  cond_cdf_out <- sapply(1:n_data, function(i){
    sapply(u_dep, function(v){length(which(data_cond_large[[i]][,dep_var] < v))/n_data_cond_large[i]})
  })
  return(cond_cdf_out)
}

## Varying u for the cdf to be calculated over
min_X <- apply(sapply(X, function(x){apply(x, 2, min)}), 1, min)
max_X <- apply(sapply(X, function(x){apply(x, 2, max)}), 1, max)
u_dep <- lapply(1:d, function(i){seq(from = min_X[i], to = max_X[i], by = 0.05)})

## cdfs of data and models
cdf_data <- lapply(1:d, function(i){lapply((1:d)[-i], function(j){
  cond_cdf_curve_data(data = X_prob_calc, u_cond = u_X[i], u_dep = u_dep[[j]], cond_var = i, dep_var = j)})})

cdf_EH <- lapply(1:d, function(i){lapply((1:d)[-i], function(j){
  cond_cdf_curve_model(data = X_EH_Original_Margins, 
                       u_cond = u_X[i], u_dep = u_dep[[j]], cond_var = i, dep_var = j)})})

cdf_Three_Step <- lapply(1:d, function(i){lapply((1:d)[-i], function(j){
  cond_cdf_curve_model(data = lapply(X_Three_Step_Graph, function(x){x$Data_Margins}), 
                       u_cond = u_X[i], u_dep = u_dep[[j]], cond_var = i, dep_var = j)})})

## get a ggplot of the CDF curves
cdf_data_all <- cdf_EH_all <- cdf_Three_Step_all <- rep(list(vector("list", d)), d)
for(i in 1:d){
  for(j in 1:d){
    if(i == j){
      cdf_data_all[[i]][[j]] <- rep(NA, length(u_dep[[j]]))
      cdf_EH_all[[i]][[j]] <- matrix(NA, nrow = length(u_dep[[j]]), ncol = n_sim)
      cdf_Three_Step_all[[i]][[j]] <- matrix(NA, nrow = length(u_dep[[j]]), ncol = n_sim)
    }
    else if(i < j){
      cdf_data_all[[i]][[j]] <- cdf_data[[i]][[j-1]]
      cdf_EH_all[[i]][[j]] <- cdf_EH[[i]][[j-1]]
      cdf_Three_Step_all[[i]][[j]] <- cdf_Three_Step[[i]][[j-1]]
    }
    else{
      cdf_data_all[[i]][[j]] <- cdf_data[[i]][[j]]
      cdf_EH_all[[i]][[j]] <- cdf_EH[[i]][[j]]
      cdf_Three_Step_all[[i]][[j]] <- cdf_Three_Step[[i]][[j]]
    }
  }
}

ci <- c(0.025, 0.975)
EH_CI <- lapply(cdf_EH_all, function(x){lapply(x, function(y){t(apply(y, 1, quantile, probs = ci, na.rm = TRUE))})})
...
ci_df <- data.frame(x_vals = rep(rep(do.call(c, lapply(u_dep, function(x){c(x, rev(x))})), n_methods), d),
                    y_vals =  c(do.call(c, lapply(EH_CI, function(x){
                      ...
                      do.call(c, lapply(Three_Step_CI, function(x){
                        do.call(c, lapply(x, function(y){c(y[,1], rev(y[,2]))}))}))),
                    ...
                    Dependent_Variable = rep(rep(do.call(c, lapply(1:d, function(i){rep(i, times = length(u_dep[[i]])*2)})), d), n_methods))

ci_df$Method <- factor(ci_df$Method, levels = methods)
ci_df$Conditioning_Variable <- factor(ci_df$Conditioning_Variable, levels = unique(ci_df$Conditioning_Variable))
ci_df$Dependent_Variable <- factor(ci_df$Dependent_Variable, levels = unique(ci_df$Dependent_Variable))

cdf_true_df <- data.frame(x_vals = rep(do.call(c, u_dep), d),
                          y_vals = do.call(c, lapply(cdf_data_all, function(x){do.call(c, x)})),
                          Conditioning_Variable = rep(1:d, each = sum(sapply(u_dep, length))),
                          Dependent_Variable = rep(do.call(c, lapply(1:d, function(i){rep(i, times = length(u_dep[[i]]))})), d))
...
ggplot() +
  ...
  scale_x_continuous(breaks = seq(from = floor(min(sapply(u_dep, min))), to = ceiling(max(sapply(u_dep, max))), by = 1)) +
  facet_grid(
...
for(i in 1:d){
  ...
  pdf(file = paste0("Images/Simulation_Study/MVT/Negative_Dependence/Probabilities/MVT_Cond_CDF_Curves_", i, ".pdf"), width = 10, height = 10)
  p <- ggplot() +
...
# saveRDS(out, file = "Data/MVT_Negative_Dependence_D5.RData")
################################################################################
