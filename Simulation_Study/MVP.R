...
Gamma_true <- complete_Gamma(c(1, 2, 1, 2, 1, 2), graph = g_true)
Theta_true_i <- lapply(1:d, function(i){Gamma2Theta(Gamma_true, k = i)})
Sigma_true_i <- lapply(Theta_true_i, solve)
rho_true_i <- lapply(Sigma_true_i, cov2cor)

## Simulate the data
X <- replicate(n = n_sim, expr = rmpareto(n = n_data, model = "HR", par = Gamma_true), simplify = FALSE)
...
fit_EH <- lapply(X, function(x){
  fmpareto_graph_HR(data = x, graph = g_true,
...
## true distribution
X_prob_calc <- rmpareto(n = 1e+7, model = "HR", par = Gamma_true)
...
X_EH <- lapply(fit_EH, function(x){rmpareto(n = n_data, model = "HR", par = x)})
...
for(i in 1:d){
  for(j in 1:length(uncon[[1]])){
    ...
    pdf(paste0("Images/Simulation_Study/MVP/Probabilities/Site_", i, "/Prob_", j, ".pdf"), height = 10, width = 10)
    par(mfrow = c(1, 1), mgp = c(2.3, 1, 0), mar = c(5, 4, 4, 2) + 0.1)
    p_plot <- ggplot() + geom_boxplot(data = data_to_plot, aes(y = Value, fill = Model)) +
      lims(y = c(ymin, max(ymax, 0.4))) +
...
## Exclude Engelke and Hitz model as it naturally is the least biased and least variable
## Since the model is built for this data specfically
## RMSE
RMSE_out <- lapply(1:d, function(i){t(sapply(1:length(uncon[[1]]), function(j){
  sapply(3:length(method_vec), function(k){
    RMSE(p_comp[[i]][[j]][,k], p_comp[[i]][[j]][,1])})}))})

RMSE_min <- sapply(RMSE_out, function(x){apply(x, 1, which.min)})

RMSE_Winner <- data.frame(matrix(0, ncol = d, nrow = length(method_vec_1[-1])))
rownames(RMSE_Winner) <- method_vec_1[-1]
for(i in 1:d){
  RMSE_Winner[as.numeric(names(table(RMSE_min[,i]))), i] <- table(RMSE_min[,i])
}
RMSE_Winner$Total <- rowSums(RMSE_Winner)
...
Bias_Winner <- data.frame(matrix(0, ncol = d, nrow = length(method_vec_1[-1])))
rownames(Bias_Winner) <- method_vec_1[-1]
for(i in 1:d){
  Bias_Winner[as.numeric(names(table(Bias_min[,i]))), i] <- table(Bias_min[,i])
}
Bias_Winner$Total <- rowSums(Bias_Winner)
