...
required_pckgs <- c("gtools", "glasso", "purrr")
...
log_like_model_MVGG <- function(fit, y){
  ...
  b <- fit$par$main[2,]
  loc <- fit$par$main[3,]
  scale <- fit$par$main[4,]
  shape <- fit$par$main[5,]
  Gamma <- fit$par$Gamma
  Sigma <- solve(as.matrix(Gamma))
  SigmaChol <- chol(Sigma)
  Z <- fit$Z
  
  ## Part 1
  ll_1 <- sum(dmvdlaplace(z = Z, mu = loc, sigmad = scale, delta = shape, 
                          SigmaChol = SigmaChol, Sigma = Sigma, log = TRUE))
...
Cond_Extremes_MVGG_Three_Step <- function(data, cond = 1, graph = NA,
  ...
  if(missing(start_GG)){
    start_GG <- matrix(rep(c(0, 1.5, 1.5), d-1), nrow = d-1, ncol = 3, byrow = TRUE)
  }
  if(!is.numeric(start_HT) | !is.numeric(start_GG)){
    stop("start_HT and start_GG must be vectors of length 2 and 3, respectively, or matrices of dimension (d - 1) x 2 and (d-1) x 3, respectively")
  }
  if(length(start_HT) == 2){
    start_HT <- matrix(rep(start_HT, d-1), ncol = 2, byrow = TRUE)
  }
  if(length(start_GG) == 3){
    start_GG <- matrix(rep(start_GG, d-1), ncol = 3, byrow = TRUE)
  }
  if(length(start_HT) != 2*(d-1) | length(start_GG) != 3*(d-1)){
    stop("start_HT and start_GG must be vectors of length 2 and 3, respectively, or matrices of dimension (d - 1) x 2 and (d-1) x 3, respectively")
  }
  ...
  if(any(start_GG[,-1] <= 0)){
    stop("Initial starting values are outside the parameter space for MVGG")
  }
  ...
  if(any(is.na(a_hat))){
    ...
    out <- list()
    out$par <- list(a = rep(NA, d), b = rep(NA, d), loc = rep(NA, d), scale = rep(NA, d), shape = rep(NA, d), Gamma = as(matrix(NA, ncol = d, nrow = d), "sparseMatrix"))
    ...
    ## to initiate
    start_GG[,1] <- apply(z, 2, mean)
    res_GG <- lapply(1:(d-1), function(i){fit_ddlaplce(par = start_GG[i,],
                                                        data = z[,i])})
    
    ## Get the necessary output
    loc_hat <- sapply(res_GG, function(x){x$par[1]})
    scale_hat <- sapply(res_GG, function(x){x$par[2]})
    shape_hat <- sapply(res_GG, function(x){x$par[3]})
    ...
    if(any(is.na(loc_hat))){
      ...
      if(!is_igraph(graph)){
        ...
        if(n_edges == n_edges_full){
          ...
          if(inherits(fit_glasso, "try-error")){
            ...
            out <- list()
            out$par <- list(loc = rep(NA, d), scale = rep(NA, d), shape = rep(NA, d), Gamma = matrix(NA, ncol = d, nrow = d))
            out$Z <- matrix(NA, nrow = n, ncol = d)
            out$convergence <- NA
          }
          else{
      ...
      ## Extract the output
      out$par$main <- as.matrix(rbind(a_hat, b_hat, loc_hat, scale_hat, shape_hat))
      out$Z <- z
      if(any(is.na(out$par$main))){
        out$par$main <- matrix(NA, nrow = 5, ncol = d-1)
        out$Z <- matrix(NA, nrow = nrow(out$Z), ncol = ncol(out$Z)) 
      }
      rownames(out$par$main) <- c("a", "b", "loc", "scale", "shape")
      ...
      out$convergence <- max(sapply(res_HT, function(x){x$convergence}),
                             sapply(res_GG, function(x){x$convergence}))  
      class(out) <- "Cond_Extremes_MVAGG"
      if(any(is.na(out$par$main))){
        out$loglike <- NA
      }
      else{
        out$loglike <- log_like_model_MVGG(out, y = yex) 
      }
    }
  }
  return(out)
